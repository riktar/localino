...
test('Quota renderer: all buckets, unknown/zero/over-limit, stale state and response-to-render', async () => {
  ...
  try {
    const page=await compactPage(app); await page.getByRole('button',{name:'Connect',exact:true}).waitFor()
    ...
    await page.locator('.agent-summary details').first().locator('summary').click()
    await page.getByText('123% used · Limit exceeded',{exact:true}).waitFor()
    assert.ok(Date.now()-start<=2000)
    assert.equal(await page.locator('[data-bucket]').count(),3)
    assert.equal(await page.getByRole('progressbar').count(),3)
    assert.equal(await page.getByRole('progressbar').nth(2).getAttribute('aria-valuenow'),'100')
    assert.equal(await page.getByText('100% remaining',{exact:true}).count(),2)
    assert.equal(await page.locator('[data-bucket="other"]').getByText('Unavailable',{exact:true}).count(),1)
    ...
    await page.getByText(/Stale/).first().waitFor()
    assert.equal(await page.getByRole('progressbar').count(),3)
    ...
    await page.waitForFunction(()=>document.querySelector('[data-bucket="codex"] time')?.textContent?.includes('12:01:00'))
    assert.match(await reset.innerText(),/0?1\/0?2\/2027/)
    await page.getByText(/in 2 min/).waitFor()
