...
test('owned Codex stream delivers 1000 deltas/s through durable storage and Ink with responsive input', {timeout:60000},async()=>{
  ...
  try{
    ...
    const visible=await card.locator('.xterm-rows').innerText()
    await card.getByRole('textbox',{name:/Message for/}).fill('FOLLOW_FIXTURE');await card.getByRole('button',{name:'Send',exact:true}).click()
    const newMessages=card.getByRole('button',{name:'New messages',exact:true});await newMessages.waitFor();await page.waitForTimeout(200);assert.equal(await card.locator('.xterm-rows').innerText(),visible);await newMessages.click()
