...
test('Real Codex account: connect, reread, restart, disconnect without logout', { timeout: 90_000 }, async () => {
  ...
  try {
    ...
    try {
      ...
      await page.getByRole('button',{name:'Settings',exact:true}).click();await page.getByText('Connected',{exact:true}).waitFor()
      ...
      await app.close()
      app = await launch(); page = await compactPage(app);await page.getByRole('button',{name:'Settings',exact:true}).click()
      await page.getByText('Connected',{exact:true}).waitFor()
