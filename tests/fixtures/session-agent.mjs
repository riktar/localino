...
input.on('line',line=>{
  ...
  if(codex){
    ...
    else if(value.type==='prompt'){const turn=piTurn++;send({id:value.id,type:'response',command:'prompt',success:true});send({type:'agent_start'});send({type:'turn_start',turnIndex:turn,timestamp:Date.now()});setTimeout(()=>{send({type:'turn_end',turnIndex:turn,message:{stopReason:'stop'}});send({type:'agent_settled'})},150)}
