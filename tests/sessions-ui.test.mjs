...
test('supervised session starts for a chosen project, survives window hiding and stops explicitly',async()=>{
  ...
  try{
    ...
    const instances=await page.locator('[data-session-instance]').evaluateAll(nodes=>nodes.map(node=>node.getAttribute('data-session-instance')))
    assert.equal(new Set(instances).size,2)
    await page.evaluate(()=>window.localino.hide())
    assert.equal((await page.evaluate(()=>window.localino.getLiveSessions())).sessions.filter(session=>session.status==='idle').length,2)
