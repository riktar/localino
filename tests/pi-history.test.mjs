...
test('Pi reader and packaged UI match official SessionManager fixture, survive source change and a large archive',async()=>{
  ...
  try{
    ...
    await page.getByRole('button',{name:'Open daily table'}).click();assert.match(await page.getByRole('table').innerText(),/34/)
    ...
    await page.getByRole('button',{name:'Disconnect',exact:true}).click();await wait(state=>!state.enabled&&state.data===null)
