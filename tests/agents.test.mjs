...
test('agent selection synchronizes windows, persists, guards drafts and validates IPC',async()=>{
  ...
  try{
    ...
    assert.match(await page.locator('#command-usage').innerText(),/Collega prima la cronologia locale/)
    assert.equal(await page.locator('#command-quotas').getAttribute('aria-disabled'),'true')
    assert.match(await page.locator('#command-quotas').innerText(),/non espone quote account universali/)
    ...
    assert.equal(await app.evaluate(()=>globalThis.agentMenu.items.find(item=>item.label==='Aggiorna statistiche').enabled),true)
    ...
    assert.equal((await page.evaluate(()=>window.localino.getAgents())).selected,'pi')
