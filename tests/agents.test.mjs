...
test('agent selection synchronizes windows, persists, guards drafts and validates IPC',async()=>{
  ...
  try{
    ...
    assert.equal(await panel.getByRole('combobox',{name:'Agent',exact:true}).inputValue(),'pi')
    await panel.getByRole('combobox',{name:'Agent',exact:true}).selectOption('claude')
