import { compactPage,packagedExecutable } from './helpers.mjs'
...
test('Real dashboard and packaged lifecycle: source comparison, shared views, hidden usage, restart and disconnect', {timeout:90000}, async()=>{
  ...
  const launch=()=>electron.launch({...(packaged?{executablePath:packagedExecutable(),args:[`--user-data-dir=${profile}`]}:{args:['.',`--user-data-dir=${profile}`]}),env})
  ...
  try {
    ...
    await page.getByText('Daily table',{exact:true}).click()
    ...
    await app.evaluate(({BrowserWindow})=>BrowserWindow.getAllWindows().find(w=>w.getTitle().includes('Usage'))!.setSize(800,600))
    ...
    await panel.getByRole('heading',{name:'Clipboard',exact:true}).waitFor()
    assert.equal((await page.evaluate(()=>window.localino.getUsage())).lastSuccessAt,beforeNavigation.lastSuccessAt)
    await panel.evaluate(()=>window.localino.openDashboard())
    ...
    await app.evaluate(({BrowserWindow})=>BrowserWindow.getAllWindows().find(w=>w.getTitle().includes('Usage'))!.close())
    await app.evaluate(()=>{const original=Date.now;Date.now=()=>original()+301000})
    await new Promise(r=>setTimeout(r,1600))
