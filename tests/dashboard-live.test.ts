...
test('Real dashboard and packaged lifecycle: source comparison, shared views, hidden usage, restart and disconnect', {timeout:90000}, async()=>{
  ...
  try {
    ...
    await app.evaluate(()=>{
      ...
      menu.items.find(item=>item.label==='Apri dashboard')!.click()
    })
    ...
    await app.evaluate(({BrowserWindow})=>BrowserWindow.getAllWindows().find(w=>w.getTitle().includes('Statistiche'))!.setSize(800,600))
    ...
    await app.evaluate(({BrowserWindow})=>BrowserWindow.getAllWindows().find(w=>w.getTitle().includes('Statistiche'))!.close())
    ...
    const restarted=await app.firstWindow();await restarted.getByRole('button',{name:'Collega Codex',exact:true}).waitFor()
