...
test('Dashboard: one secure window, sparse data, filters, keyboard, credits and independent errors at minimum size',async()=>{
  ...
  try {
    const panel=await compactPage(app);await panel.getByRole('button',{name:'Advanced usage',exact:true}).click()
    const page=app.windows().find(w=>w!==panel)??await app.waitForEvent('window')
    const errors=[];page.on('pageerror',error=>errors.push(error.message))
    await page.getByRole('heading',{name:'Codex usage',exact:true}).waitFor()
    ...
    await app.evaluate(({BrowserWindow})=>{const w=BrowserWindow.getAllWindows().find(w=>w.getTitle().includes('Usage')) ;w.setSize(800,600)})
    ...
    await page.getByText('Service credits',{exact:true}).click()
    await page.getByText('Quota details',{exact:true}).click()
    await page.getByText('Unlimited credits',{exact:true}).waitFor()
    assert.equal(await page.getByRole('combobox',{name:'Period'}).inputValue(),'30')
    ...
    assert.match(await page.locator('[data-period-metrics]').innerText(),/Partial total\s+30/)
    assert.match(await page.locator('[data-period-metrics]').innerText(),/Mean per reported day\s+10/)
    ...
    await app.evaluate(({BrowserWindow})=>BrowserWindow.getAllWindows().find(w=>w.getTitle().includes('Usage')).focus())
    ...
    await page.getByText('Daily table',{exact:true}).focus();await page.keyboard.press('Enter')
    ...
    await page.getByRole('combobox',{name:'Period'}).focus();await page.keyboard.press('End');await page.keyboard.press('Enter')
    assert.equal(await page.getByRole('combobox',{name:'Period'}).inputValue(),'all')
    assert.match(await page.locator('[data-summary]').innerText(),/1,?000/)
    ...
    assert.equal(await app.evaluate(({BrowserWindow})=>BrowserWindow.getAllWindows().find(w=>w.getTitle().includes('Usage')).isVisible()),false)
    ...
    await page.getByText("Connect Codex to view usage.",{exact:true}).waitFor()
