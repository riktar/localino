...
export function Shell(): React.JSX.Element {
  ...
  return <div className="min-h-screen">
    ...
    <div ref={content} tabIndex={-1} className="outline-none" data-destination={destination}>
      {destination === 'consumi' ? <Dashboard /> : <main className="mx-auto max-w-5xl space-y-8 p-6 lg:p-10">
        {destination === 'home' ? <>
        ...
        </> : <><h1 className="text-3xl font-semibold">{destinationLabels[destination]}</h1><p className="text-muted-foreground">{destination === 'clipboard' ? 'La libreria di prompt sarà disponibile con il completamento di questo incremento.' : 'Naviga con Tab e attiva i pulsanti con Invio. Le scorciatoie configurabili saranno disponibili con il completamento di questo incremento.'}</p></>}
