...
if (!app.requestSingleInstanceLock()) {
  ...
  app.whenReady().then(async () => {
    ...
    powerMonitor.on('suspend', () => { capture.suspend(); quotas.suspend(); usage.suspend();liveSessions.suspend();Object.values(histories).forEach(r=>r.suspend()) })
    powerMonitor.on('resume', () => { capture.resume(); quotas.resume(); usage.resume();liveSessions.resume();Object.values(histories).forEach(r=>r.resume()) })
