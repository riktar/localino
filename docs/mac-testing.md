...
- For each installed Codex, Claude Code, Pi or OpenCode CLI, confirm that **Start session** discovers the local binary, asks for a project and starts exactly one Localino-owned process without opening Terminal. Record the CLI version and protocol. An unavailable binary must remain explicitly unavailable and must not be installed automatically.
- Start two supervised sessions for the same agent and project plus one for another installed agent. Confirm distinct Localino IDs, independent state/timers and exact routing when selection changes during a send. Hiding or closing the panel must keep all three sessions alive; **Stop** must end only the selected owned process.
- Send multiline Unicode text while idle, then queue two messages during a turn. Confirm the receipt and terminal lifecycle are distinct, FIFO order is preserved and a queued message can be cancelled. Simulate a lost connection after write: the in-flight delivery must become `Unknown`, remaining entries `Suspended`, and neither may be retried automatically.
- With an unsent composer draft and unresolved delivery, quit and relaunch Localino. Confirm the recovered text is shown under **Recovered drafts and undelivered messages**, no CLI starts and no message is sent. Quit must leave no Localino-owned CLI process behind and must not terminate unrelated agent processes.
